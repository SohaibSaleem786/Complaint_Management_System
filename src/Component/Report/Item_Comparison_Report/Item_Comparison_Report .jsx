...
const Item_Comparison_Report  = () => {
  ...
  useEffect(() => {
    fetch(`${apiLinks}/ItemComplaintReport.php`)
      ...
      .then((apiData) => {
        ...
        const columns = [
          ...
          { label: "Hold ", field: "taredsc", sort: "asc" },
...
const blankRows = Array.from({ length: Math.max(0, numberOfRows - filteredRows.length) }).map((_, index) => (
  ...
  return (
    <>
      <Header />
      <PathHead
        pageName="Report > Item Comparison Report"
        screen="Get_Item"
        pageLink="/MainPage"
      />
      ...
      <div className="col-12" style={{ color: secondaryColor }}>
        <br />
        <div
          className="Item_Comparison_Report"
          style={{
            // marginLeft: "30%",
            // marginRight: "30%",
            // maxWidth: "40%",
...
export default Item_Comparison_Report ;
