...
const Closed_Complaint = () => {
    ...
    fetch(`${apiLinks}/GetComplaint.php`)
      ...
      .then((apiData) => {
        ...
        const columns = [
          ...
          { label: "Closed", field: "itmdscurd", sort: "asc" },
  ...
  const handleRowClick = (row) => {
    if (selectedRow === row.c_id) {
      ...
      navigate(`/Update_ClosedComplaint/${row.c_id}`);
    } else {
...
const blankRows = Array.from({ length: Math.max(0, numberOfRows - filteredRows.length) }).map((_, index) => (
  ...
  return (
    <>
      <Header />
      <PathHead
        pageName="Transaction > Closed Complaint "
        screen="Get_Item"
        pageLink="/MainPage"
      />
