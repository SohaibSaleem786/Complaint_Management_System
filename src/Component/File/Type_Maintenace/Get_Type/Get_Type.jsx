...
const Get_Type = () => {
    ...
    fetch(`${apiLinks}/GetType.php`)
      ...
      .then((apiData) => {
        ...
        const columns = [
          ...
          { label: "Status ", field: "itmdscurd", sort: "asc" },
        ];
  ...
  const filteredRows = data.rows.filter(
    (row) =>
      ...
      (row.typsts &&
        row.typsts.toLowerCase().includes(searchText.toLowerCase()))
  );
...
const blankRows = Array.from({ length: Math.max(0, numberOfRows - filteredRows.length) }).map((_, index) => (
  ...
  return (
    <>
      <Header />
      <PathHead
        pageName="File > Type Maintenance"
        screen="Get_Item"
        pageLink="/MainPage"
      />
